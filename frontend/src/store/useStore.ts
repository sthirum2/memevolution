...
import type { AgentState, CorpusStats, EvolveResult, Experiment, GenerationSummary, LiveMetrics, SelectionResult } from '@/types'
...
interface Store {
  ...
  view: View; selectedGen: number; openId: string | null; lab: LabState
...
export const useStore = create<Store>((set, get) => ({
  ...
  view: 'evolution', selectedGen: 0, openId: null, lab: freshLab(),
  async init() {
    if (booting) return
    booting = true
    try {
